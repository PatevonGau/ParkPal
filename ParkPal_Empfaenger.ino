...
public:
    ...
    void controlBuzzer(int distance1, int distance2, int distance3) {
        ...
        if (minDistance > 80) {
            ...
            int duration;
            if (minDistance <= 30) {
                duration = map(minDistance, 10, 30, 80, 120); // Dauer für Entfernungen <= 30 cm
            } else {
                duration = map(minDistance, 31, 80, 120, 350); // Dauer für Entfernungen > 30 cm
            }
            int frequency = map(minDistance, 10, 80, 450, 430); // Frequenz abhängig von Entfernung
...
void sendHeartbeat(unsigned long currentMillis) {
    if (currentMillis - lastDataReceivedTime > heartbeatInterval) {
        ...
        if (ackReceived) {
            radioConnected = true;  // Verbindung erfolgreich
            lastDataReceivedTime = currentMillis;  // Zeitpunkt aktualisieren
        } else {
...
void handleRadioCommunication() {
    ...
    if (radio.available()) {
        ...
        Serial.print(F("Updated sensor distances: ")); // Ausgabe der Sensorwerte über Serial
        for (int i = 0; i < 3; i++) {
